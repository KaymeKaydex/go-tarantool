...
// PrivilegeObjectType is a struct based on privilege object types list
...
const (
	// PrivilegeObjectTypeUniverse - privilege type based on universe.
	...
	PrivilegeObjectTypeUniverse PrivilegeObjectType = "universe"
	// PrivilegeObjectTypeUser - privilege type based on user.
	// A user identifies a person or program that interacts with a Tarantool instance.
	PrivilegeObjectTypeUser PrivilegeObjectType = "user"
	// PrivilegeObjectTypeRole - privilege type based on role.
	...
	PrivilegeObjectTypeRole PrivilegeObjectType = "role"
	// PrivilegeObjectTypeSpace - privilege type based on space.
	// Tarantool stores tuples in containers called spaces.
	PrivilegeObjectTypeSpace PrivilegeObjectType = "space"
	// PrivilegeObjectTypeFunction - privilege type based on functions.
	// This allows access control based on function access.
	PrivilegeObjectTypeFunction PrivilegeObjectType = "function"
	// PrivilegeObjectTypeSequence - privilege type based on sequences.
	// A sequence is a generator of ordered integer values.
	PrivilegeObjectTypeSequence PrivilegeObjectType = "sequence"
	// PrivilegeObjectTypeLuaEval - privilege type based on executing arbitrary Lua code.
	PrivilegeObjectTypeLuaEval PrivilegeObjectType = "lua_eval"
	// PrivilegeObjectTypeLuaCall - privilege type based on
	// calling any global user-defined Lua function.
	PrivilegeObjectTypeLuaCall PrivilegeObjectType = "lua_call"
	// PrivilegeObjectTypeSQL - privilege type based on
	// executing an arbitrary SQL expression.
	PrivilegeObjectTypeSQL PrivilegeObjectType = "sql"
)
...
type Privilege struct {
	...
	// ObjectType - one of privilege object types (it might be space,function, etc.).
	ObjectType PrivilegeObjectType
	// ObjectName - can be the name of a function or space,
...
func (uer *UserInfoResponse) DecodeMsgpack(d *msgpack.Decoder) error {
	...
	for i, rawPrivileges := range rawArr[0] {
		...
		privileges[i] = Privilege{
			Permissions: perms,
			ObjectType:  PrivilegeObjectType(rawPrivileges[1]),
			ObjectName:  rawPrivileges[2],
		}
	}
...
func prepareGrantAndRevokeArgs[T UserGrantOptions | UserRevokeOptions](username string,
	...
	case PrivilegeObjectTypeUniverse:
		...
		args = append(args, reqPerms, string(privilege.ObjectType), nil, opts)
	case PrivilegeObjectTypeRole:
		// Handling the case where the object type is a role name.
		// Append role-specific arguments to args.
		args = append(args, privilege.ObjectName, nil, nil, opts)
	default:
		...
		args = append(args, reqPerms, string(privilege.ObjectType), privilege.ObjectName, opts)
	}
